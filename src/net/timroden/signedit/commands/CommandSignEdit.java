	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args[0].equalsIgnoreCase("copy")) {
			...
			if (args.length > 1) {
				if (args[1].equalsIgnoreCase("persist")) {
					...
					player.sendMessage(this.plugin.chatPrefix + this.plugin.localization.get("persistEnabled", new Object[] { this.utils.capitalize(this.plugin.config.clickActionStr()) }));
					return true;
				}
				...
				if (!this.utils.isInt(args[1])) {
					...
					player.sendMessage(this.plugin.chatPrefix + this.plugin.localization.get("defaultCopyPunch", new Object[] { this.utils.capitalize(this.plugin.config.clickActionStr()), this.plugin.pasteAmounts.get(player.getName()), ((Integer) this.plugin.pasteAmounts.get(player.getName())).intValue() == 1 ? this.plugin.localization.get("pasteCopyStr") : this.plugin.localization.get("pasteCopiesStr") }));
					return true;
				}
				SignEditDataPackage tmp = new SignEditDataPackage(player.getName(), SignFunction.COPY, Integer.parseInt(args[1]));
				this.plugin.playerData.put(player.getName(), tmp);
				player.sendMessage(this.plugin.chatPrefix + this.plugin.localization.get("intCopyPunch", new Object[] { this.utils.capitalize(this.plugin.config.clickActionStr()), args[1], Integer.parseInt(args[1]) == 1 ? this.plugin.localization.get("pasteCopyStr") : this.plugin.localization.get("pasteCopiesStr") }));
				return true;
			}
		...
		if (args.length >= 1) {
			...
			if (line == null) {
				...
				this.plugin.playerData.put(player.getName(), tmp);
				player.sendMessage(this.plugin.chatPrefix + this.plugin.localization.get("punchToComplete", new Object[] { this.utils.capitalize(this.plugin.config.clickActionStr()) }));
				return true;
			}
			if (line.length() > 15) {
